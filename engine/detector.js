 ...
 * Wrapped in UMD/IIFE to prevent global variable collision in browser scripts.
 */
(function(root) {
      ...
      headings.forEach((heading) => {
        ...
        if (prevLevel > 0 && currentLevel > prevLevel + 1) {
          violations.push({
            ...
            recommendation: `Change <h${currentLevel}> to <h${prevLevel + 1}>.`,
          ...
          violations.push({
            ...
            selector: heading.tagName.toLowerCase(),
            elementHtml: heading.outerHTML,
            description: `First heading in document is <h${currentLevel}>. Outline should start at <h1> or <h2>.`,
            category: 'deterministic',
            engine: 'Deterministic Logic',
      ...
      images.forEach(img => {
        ...
        if (!hasAlt || (!isDecorative && altVal !== null && altVal.trim() === '')) {
          violations.push({
            ...
            description: hasAlt ? 'Image has empty alt attribute on informative image.' : 'Image is missing alternative text (alt attribute).',
            category: 'ai_interpretation',
            engine: 'AI / LLM Semantic Model',
            recommendation: 'Use contextual AI to generate meaningful alternative text.',
      ...
      buttons.forEach(btn => {
        ...
        if (!text && !hasAria) {
          violations.push({
            ...
            category: 'deterministic',
            engine: 'Deterministic Logic',
            recommendation: 'Provide discernible text or aria-label.',
            data: {}
          });
        }
      });
      ...
      links.forEach(a => {
        ...
        if (!text && !hasAria && !imgWithAlt) {
          violations.push({
            ...
            description: 'Link element has no discernible text or label.',
            category: 'deterministic',
            engine: 'Deterministic Logic',
          ...
          if (fg && bg) {
            ...
            if (ratio < 4.5) {
              violations.push({
                ...
                description: `Insufficient text contrast ratio: ${ratio.toFixed(2)}:1 (Expected >= 4.5:1).`,
                category: 'deterministic',
